        ...
        <form class="card-body" @submit.prevent="submitForm">
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    ...
                    <span v-if="individualSuggestion('email_address')" class="badge bg-secondary mt-1" style="cursor:pointer" @click="applyIndividualField('email', 'email_address')" title="Click to use this value">{{ individualSuggestion('email_address') }}</span>
                </div>
                <div class="col-md-3">
...
export default {
    ...
    computed: {
        individual() {
            return this.results?.individual_data?.individual ?? null;
        }
    },
    methods: {
        individualSuggestion(individualKey) {
            if (!this.individual) return null;
            const val = this.individual[individualKey];
            return val !== null && val !== undefined && val !== '' ? val : null;
        },
        applyIndividualField(formKey, individualKey) {
            const val = this.individualSuggestion(individualKey);
            if (val !== null) {
                this.editForm[formKey] = val;
            }
        },
